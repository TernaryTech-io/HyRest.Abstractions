...
public abstract class OnBaseModule<TApi> : IOnBaseModule
    where TApi : IHylandRestAPI
{
    ...
    public TApi Api => _app.ClientFactory.CreateClient<TApi>();
        ...
        where TAPI : IHylandRestAPI
    {
        ...
        while (!token.IsCancellationRequested)
        {
            ...
            if (Api is TAPI a)
                ...
                api = App.ClientFactory.CreateClient<TAPI>();
            res = await function(api, token);
            var exception = HandleResponse<IApiResponse>(res, token).WaitAsync(App.RequestTimeOut).Result;
            if (exception != null)
        ...
        where TOut : class
    {
        ...
        while(!token.IsCancellationRequested)
        {
            ...
            if (Api is TAPI a)
                ...
                api = App.ClientFactory.CreateClient<TAPI>();
            var task = await function(api, token);
            var exception = HandleResponse<TOut>(res, token).WaitAsync(App.RequestTimeOut).Result;
            if (exception != null)
                throw exception.InnerException?.InnerException ?? exception.InnerException ?? exception;
    ...
    private void ExtractCookie(IApiResponse response)
    {
        if(response.Headers != null && response.Headers.Any(h => h.Key == "Set-Cookie"))
        {
            ...
            if(App.ClientFactory.ApiClient.CookieContainer != null)
                App.ClientFactory.ApiClient.CookieContainer.SetCookies(new Uri(App.ClientOptions.ApiBaseUrl), setcookies.First().Value.First());
        }
        return;
    }
