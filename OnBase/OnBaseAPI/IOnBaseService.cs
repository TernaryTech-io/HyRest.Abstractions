    ...
    where TApi : class, IHylandRestAPI
{
    ...
    private readonly IOnBaseClientFactory _hylandClientFactory;
    private readonly IOnBaseAppCache _cache;
    protected string? CachePrefix => _hylandClientFactory?.AuthClient?.UserInfo?.ToString();
    public OnBaseService(IOnBaseAppCache cache, IOnBaseClientFactory hylandClientFactory, ILogger<IOnBaseService> logger)
    {
    ...
    public IOnBaseClientFactory ClientFactory => _hylandClientFactory;
...
public interface IOnBaseService
{
    ...
    IOnBaseClientFactory ClientFactory { get; }
